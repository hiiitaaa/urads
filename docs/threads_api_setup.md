...
Facebookページ作成後、以下にアクセス:

1. https://developers.facebook.com/ を開く
2. 右上の **「開始する」** をクリック
3. 開発者規約に同意
4. アプリダッシュボードが表示されればOK
...
2. **「アプリを作成」** をクリック
3. ユースケースで **「Access Threads API」**（Threads APIにアクセス）を選択
4. アプリ名を入力（例: `Urads`）
5. 連絡先メールを入力

### 3.2 ビジネスポートフォリオ接続（任意）

- 個人開発なら「今はスキップ」でOK
- 有料配布する場合は後でビジネス検証が必要になる
...
アプリダッシュボード → **ユースケース** → **Access Threads API** → **Settings**
...
同じ Settings 画面で、以下の3つのURLを**全て**入力する（全部入れないと保存できない）:
...
アプリダッシュボード → **アプリの役割** → **役割** タブ

- 「Threadsテスター」として自分や他のアカウントを追加
- 招待されたユーザーは **Threadsアプリの設定画面** で招待を承認する必要がある
- アプリ審査を通さなくても、テスターに追加したアカウントでAPIが使える
...
**ここに表示されるのはFacebook用のApp ID。Threads用ではない。**

Threads用のApp IDとApp Secretは:
- ダッシュボード → **ユースケース** → **Access Threads API** → **Settings**

ここにThreads専用の `App ID` と `App Secret` が表示される。
