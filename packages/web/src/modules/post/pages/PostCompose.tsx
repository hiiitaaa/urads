...
export function PostCompose(): React.JSX.Element {
  ...
  const [mediaFiles, setMediaFiles] = useState<Array<{ id: string; name: string; url?: string; key?: string; type?: string; uploading?: boolean }>>([]);
  const maxLength = 500;
  ...
  const uploadFile = async (file: File) => {
    ...
    setMediaFiles((prev) => [...prev, { id: fileId, name: file.name, type, uploading: true }]);
    ...
    try {
      ...
      const result = await res.json() as { key?: string; url?: string; type?: string; error?: string };

      if (res.ok && result.url) {
        setMediaFiles((prev) => prev.map((f) =>
          f.id === fileId ? { ...f, url: result.url, key: result.key, type: result.type || type, uploading: false } : f
        ));
      } else {
    ...
    try {
      ...
      if (data.status === 'posted') {
        ...
        for (const f of mediaFiles) {
          if (f.key) apiFetch(`/media/${f.key}`, { method: 'DELETE' }).catch(() => {});
        }
        setMediaFiles([]);
