...
export function ReplyRules({ preselectedPostId, onClearPreselect }: Props): React.JSX.Element {
  ...
  const loadData = (accountId: string) => {
    // ルール一覧
    fetch(`${API_BASE}/replies/rules?account_id=${accountId}`)
      .then((r) => r.json())
      .then((data) => setRules(data.rules || []))
      .catch(() => setRules([]));

    // 投稿一覧（posted のみ）
    fetch(`${API_BASE}/posts?account_id=${accountId}&status=posted`)
      .then((r) => r.json())
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/replies/rules`, {
        method: 'POST',
  ...
  const handleToggle = async (id: string, currentActive: number) => {
    await fetch(`${API_BASE}/replies/rules/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`${API_BASE}/replies/rules/${id}`, { method: 'DELETE' });
    loadData(selectedAccount);
  };
