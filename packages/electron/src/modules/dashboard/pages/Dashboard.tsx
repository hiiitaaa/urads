...
export function Dashboard(): React.JSX.Element {
  ...
  const fetchInsights = async (accountId: string) => {
    try {
      const res = await fetch(`${API_BASE}/posts/insights?account_id=${accountId}`);
  ...
  const handleRefresh = async () => {
    ...
    setError('');
    try {
      const result = await window.urads.insightsRefresh(account.threads_handle, account.id);
      if (result && !result.ok) {
        setError(result.error || 'リフレッシュに失敗しました');
      } else {
        await fetchInsights(account.id);
      }
    } catch (e) {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <button
          ...
          style={{
            ...
            background: refreshing ? '#f0f0f0' : '#fff', cursor: refreshing ? 'default' : 'pointer',
            fontSize: 13,
          }}
        >
          {refreshing ? '取得中...' : 'Insightsを更新'}
        </button>
      </div>
